...
#
# Modified by Unimedvl Team with enhancements including:
# - Distributed training support for multi-GPU training
# - Grouped dataset configuration for flexible data management
# - Deterministic random sampling for reproducibility
# - Refactored modular architecture for maintainability
...
class PackedDataset(torch.utils.data.IterableDataset):
    ...
    def build_datasets(self, datasets_metainfo, data_status):
        ...
        for grouped_dataset_name, dataset_args in datasets_metainfo.items():
            print(f"dataset_name:{grouped_dataset_name}")
            print(f" with args: {dataset_args}")
            print("--------------------------------")
            
            dataset_names = dataset_args.get('dataset_names')
            if dataset_names and isinstance(dataset_names, dict):
                print(f"Using new grouped format for {grouped_dataset_name}")
                
                weight_config = dataset_args.get('weight', 1.0)
                num_used_data_config = dataset_args.get('num_used_data', {})
                is_mandatory_flag = dataset_args.get('is_mandatory', False)
                
                for group_name, group_dataset_names in dataset_names.items():
                    print(f"Processing group: {group_name}")
                    
                    group_dataset_args = dataset_args.copy()
                    
                    group_dataset_args['dataset_names'] = group_dataset_names
                    
                    if isinstance(weight_config, dict):
                        group_weight = weight_config.get(group_name, 1.0)
                    else:
                        group_weight = weight_config
                    
                    if isinstance(num_used_data_config, dict):
                        group_num_used_data = num_used_data_config.get(group_name, [0] * len(group_dataset_names))
                    else:
                        group_num_used_data = num_used_data_config
                    group_dataset_args['num_used_data'] = group_num_used_data
                    
                    group_dataset_args.pop('weight', None)
                    group_dataset_args.pop('is_mandatory', None)
                    
                    group_dataset_name = f"{grouped_dataset_name}_{group_name}"
                    
                    processed_dataset = self._process_single_dataset_group(
                        group_dataset_name, grouped_dataset_name, group_dataset_args, data_status
                    )
                    
                    if processed_dataset is not None:
                        datasets.append(processed_dataset)
                        is_mandatory.append(is_mandatory_flag)
                        grouped_weights.append(group_weight)
            else:
                print(f"Using traditional format for {grouped_dataset_name}")
                
                group_weight = dataset_args.pop('weight', 1.0)
                group_is_mandatory = dataset_args.pop('is_mandatory', False)
                
                processed_dataset = self._process_single_dataset_group(
                    grouped_dataset_name, grouped_dataset_name, dataset_args, data_status
                )
                
                if processed_dataset is not None:
                    datasets.append(processed_dataset)
                    is_mandatory.append(group_is_mandatory)
                    grouped_weights.append(group_weight)

        return datasets, is_mandatory, grouped_weights
    
    def _process_single_dataset_group(self, group_dataset_name, base_dataset_name, dataset_args, data_status):
        """"""
        ...
        if 'dataset_names' not in dataset_args.keys():
            print(f"Warning: No dataset_names found for {group_dataset_name}")
            return None
        ...
        for item in dataset_names:
            if self.local_rank == 0:
                print(f'Preparing Dataset {group_dataset_name}/{item}')
            meta_info = DATASET_INFO[base_dataset_name][item]
            dataset_args['data_dir_list'].append(meta_info['data_dir'] if 'data_dir' in meta_info else "")
        ...
        if data_status is not None and group_dataset_name in data_status.keys() and resume_data_status:
            data_status_per_group = data_status[group_dataset_name]
        else:
            data_status_per_group = None
            
        dataset = DATASET_REGISTRY[base_dataset_name](
    ...
    def __iter__(self):
        ...
        while True:
            ...
            """
            When processing multimodal or complex sequences, the model needs to clearly know the start and end positions of each data segment. 
            This is typically achieved by inserting special BOS (Begin of Sequence) and EOS (End of Sequence) tokens.
            So, the * 2 here reserves 1 start token and 1 end token position for each segment in sequence_plan.
            """
            ...
            if sequence_status['curr'] + num_tokens > self.max_num_tokens:
                if len(buffer) < self.max_buffer_size and not sample_from_buffer:
                    ...
                    batch_count += 1
                    if dist.is_initialized() and batch_count % 10 == 0:
                        total_length = sum(sequence_status['sample_lens'])
                        ## print(f"[DataBoundary] Rank {dist.get_rank()}: Generated batch {batch_count} - length {total_length}")
            ...
            if sequence_status['curr'] >= self.expected_num_tokens:
                ...
                if dist.is_initialized() and batch_count % 10 == 0:
                    total_length = sum(sequence_status['sample_lens'])
                   # print(f"[DataBoundary] Rank {dist.get_rank()}: Expected length batch {batch_count} - length {total_length}")
    ...
    def pack_sequence(self, sample, sequence_status):
        ...
        sample_key = str(sample.get('data_indexes', ''))
        deterministic_seed = hash(sample_key) % (2**32)
        # print(f"------------->> [Rank {dist.get_rank()}] Sample key: {sample_key}, Deterministic seed: {deterministic_seed}")
        ...
        if dist.is_initialized():
            current_sample_count = len(sequence_status.get('sample_lens', []))
            if current_sample_count % 100 == 0:
                total_processed_tokens = sequence_status['curr']
               # print(f"[DataBoundary] Rank {dist.get_rank()}: Processed {current_sample_count} samples - Total tokens: {total_processed_tokens}")
