...
def prepare_attention_mask_per_sample(split_lens, attn_modes, device="cpu"):
    """Prepare attention mask for each sample with different attention modes.

    attn_modes options: 'causal', 'full', 'noise'
    """
...
def pil_img2rgb(image):
    try:
        ...
        if total_pixels > 20_000_000:
            raise ValueError(f"Image too large: {total_pixels} pixels")
        ...
        print(f"Error in pil_img2rgb: {e}")
        print(f"Image info: mode={getattr(image, 'mode', 'unknown')}, size={getattr(image, 'size', 'unknown')}")
        raise
