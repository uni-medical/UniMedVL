...
# Copyright (c) 2025 Unimedvl Team
...
def pad_sequence(tensor, pad_size):
    if pad_size <= 0:
        return tensor
    return torch.nn.functional.pad(tensor, (0, 0, 0, pad_size, 0, 0))
...
class PackedAttention(Qwen2Attention):
    ...
    def forward_train(
        ...
        if isinstance(attention_mask, List):
            ...
            packed_attn_output = packed_query_states.new_zeros(
                (self.num_heads, total_seq_len, self.head_dim),
                dtype=torch.bfloat16
            )

            start_idx = 0
            for query_states, key_states, value_states, attention_mask_per_sample, seq_len in zip(
                unpacked_query_states, unpacked_key_states, unpacked_value_states, attention_mask, sample_lens
            ):
                ...
                packed_attn_output[:, start_idx:end_idx, :] = attn_output.squeeze(0)
                start_idx = end_idx
        else:
    ...
    def forward_inference(
        ...
        return packed_attn_output, past_keye_values
...
class PackedAttentionMoT(Qwen2Attention):
    ...
    def forward_train(
        ...
        max_valid_index = packed_sequence.shape[0] - 1
        if packed_und_token_indexes.numel() > 0 and packed_und_token_indexes.max() > max_valid_index:
            print(f"ERROR: packed_und_token_indexes contains invalid index {packed_und_token_indexes.max()}, max valid = {max_valid_index}")
        if packed_gen_token_indexes.numel() > 0 and packed_gen_token_indexes.max() > max_valid_index:
            print(f"ERROR: packed_gen_token_indexes contains invalid index {packed_gen_token_indexes.max()}, max valid = {max_valid_index}")
        ...
        if isinstance(attention_mask, List):
            ...
            total_seq_len = sum(sample_lens)
            packed_attn_output = packed_query_states_.new_zeros(
                (self.num_heads, total_seq_len, self.head_dim),
                dtype=torch.bfloat16
            )
...
class Qwen2MoTDecoderLayer(nn.Module):
    ...
    def forward_train(
        ...
        if packed_und_token_indexes.device != device:
            packed_und_token_indexes = packed_und_token_indexes.to(device)
        if packed_gen_token_indexes.device != device:
            packed_gen_token_indexes = packed_gen_token_indexes.to(device)

        valid_und_mask = (packed_und_token_indexes >= 0) & (packed_und_token_indexes < seq_len)
        valid_gen_mask = (packed_gen_token_indexes >= 0) & (packed_gen_token_indexes < seq_len)
        
        if not valid_und_mask.all():
            invalid_count = (~valid_und_mask).sum().item()
            print(f"WARNING: Found {invalid_count} invalid und indexes, sequence length: {seq_len}")
            print(f"Invalid index range: [{packed_und_token_indexes.min().item()}, {packed_und_token_indexes.max().item()}]")
            packed_und_token_indexes = packed_und_token_indexes[valid_und_mask]

        if not valid_gen_mask.all():
            invalid_count = (~valid_gen_mask).sum().item()
            print(f"WARNING: Found {invalid_count} invalid gen indexes, sequence length: {seq_len}")
            print(f"Invalid index range: [{packed_gen_token_indexes.min().item()}, {packed_gen_token_indexes.max().item()}]")
            packed_gen_token_indexes = packed_gen_token_indexes[valid_gen_mask]

        packed_sequence_ = packed_sequence.new_zeros(packed_sequence.shape)

        if packed_und_token_indexes.numel() > 0:
            try:
                und_input = packed_sequence[packed_und_token_indexes]
                und_normalized = self.input_layernorm(und_input)
                packed_sequence_[packed_und_token_indexes] = und_normalized
            except RuntimeError as e:
                print(f"ERROR: und indexing failed: {e}")
                print(f"packed_sequence.shape: {packed_sequence.shape}")
                print(f"packed_und_token_indexes.shape: {packed_und_token_indexes.shape}")
                print(f"packed_und_token_indexes range: [{packed_und_token_indexes.min()}, {packed_und_token_indexes.max()}]")
                raise

        if packed_gen_token_indexes.numel() > 0:
            try:
                gen_input = packed_sequence[packed_gen_token_indexes]
                gen_normalized = self.input_layernorm_moe_gen(gen_input)
                packed_sequence_[packed_gen_token_indexes] = gen_normalized
            except RuntimeError as e:
                print(f"ERROR: gen indexing failed: {e}")
                print(f"packed_sequence.shape: {packed_sequence.shape}")
                print(f"packed_gen_token_indexes.shape: {packed_gen_token_indexes.shape}")
                print(f"packed_gen_token_indexes range: [{packed_gen_token_indexes.min()}, {packed_gen_token_indexes.max()}]")
                raise
...
class SimpleHiddenStatesOutput:
    def __init__(self, last_hidden_state, diffusion_features):
...
class Qwen2Model(Qwen2PreTrainedModel):
    ...
    def forward_train(
        ...
        target_hidden_layer = diffusion_feature_layer if output_hidden_states else None
        ...
        for i, decoder_layer in enumerate(self.layers):
            ...
            if output_hidden_states and target_hidden_layer is not None and i == target_hidden_layer:
                diffusion_features = packed_sequence
        ...
        if self.use_moe:
            ...
            final_hidden_state = packed_sequence_
        else:
            final_hidden_state = self.norm(packed_sequence)

        if output_hidden_states and diffusion_features is not None:
            return SimpleHiddenStatesOutput(final_hidden_state, diffusion_features)
        else:
